...
export const useProductsStore = defineStore('products',() => {
  ...
  async function searchProductsBy(value: string) {
    const { data, error }: any = await supabase
      .from('products')
      .select('*')
      .ilike('name', `%${value}%`);
  ...
  return { products, reloadProducts, addProduct, getProductWithId, sortProductsBy, filterProductsBy, getTotalProducts, searchProductsBy };
});